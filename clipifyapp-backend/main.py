...
app = modal.App("clipifyapp-backend", image=image)

volume = modal.Volume.from_name(
    "clipifyapp-backend-model-cache", create_if_missing=True
)
...
def process_clip(base_dir: str, video_path: str, s3_key: str, start_time: float, end_time: float, index: int, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(
        subtitle_output_path, "clipify-backend", output_s3_key)
...
@app.cls(gpu="L40S", timeout=900, retries=0, scaledown_window=20, secrets=[modal.Secret.from_name("clipifyapp-backend-secret")], volumes={mount_path: volume})
class ClipifyAppBackend:
    @modal.enter()
    def load_model(self):
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("clipify-backend", s3_key, str(video_path))

        # 1. Transcribe video
        transcript_segments_json = self.transcribe_video(base_dir, video_path)
        transcript_segments = json.loads(transcript_segments_json)

        # 2. Identify the segments that are relevant to the user
...
def main():
    ...
    clipifyapp_backend = ClipifyAppBackend()

    url = clipifyapp_backend.process_video.web_url

    payload = {
        "s3_key": "test1/prakharxsadhguru5mins.mp4"
    }
